...
thundercomm 有提供 audio 測試用指令，[連結](https://www.thundercomm.com/rubik-pi-3/en/docs/rubik-pi-3-user-manual/1.1.2/peripherals-and-interfaces#audio)在這。
...
PAL（Platform Audio Layer）是 Qualcomm 提供的音頻抽象 API 層，位於 PipeWire/PulseAudio 和 AGM/Kernel 之間。開發者可直接呼叫 PAL API 撰寫高效率的音頻應用，無需透過 PipeWire 框架。
...
# 嘗試直接播放至 ES8316 路徑
tinyplay '/Wii_Music(48k).wav' -D 0 -d 2

tinyplay '/Wii_Music(44.1k).wav' -D 0 -d 2
failed to open for pcm 0,2. cannot set hw params: Invalid argument
```
...
```
playing '/Wii_Music(48k).wav': 2 ch, 48000 hz, 16-bit signed PCM
...
> **原因分析**
>
> **LPAIF（Low-Power Audio InterFace）** 是 Qualcomm SoC 內部負責搬運音訊資料的硬體匯流排控制器。  
> 你可以把它想像成 SoC 晶片內部的「音訊資料高速公路」：
> - 播放時，它把 DSP（ADSP Q6）處理完的數位音訊資料，透過 I2S/TDM/MI2S 介面傳送給外部 Codec（ES8316）轉成類比聲音。
> - 錄音時，方向相反——從 Codec ADC 收進來的數位資料，透過 LPAIF 送進 DSP 緩衝區。
>
> 在 Linux ALSA 架構中，`/dev/snd/pcmC0D2p` 這類節點就是 LPAIF 的 **後端（Back-End）PCM 裝置**——它代表「通往 LPAIF 硬體的門」。
>
> 問題在於：Qualcomm 的 **AudioReach** 架構規定，這扇「門」不能直接打開。  
> 必須先透過 **PAL（Platform Audio Layer）** 完成一系列初始化：
> 1. 根據 use case 選擇正確的 mixer path（例如「耳機播放」要打開哪些 ALSA mixer switch）
> 2. 呼叫 TinyALSA 以正確參數設定 LPAIF 硬體暫存器
> 3. 通知 ADSP 準備對應的音訊處理 pipeline
>
> `tinyplay` 這類工具會直接對 `hw:0,2` 發出 `open → set_hw_params → write` 的系統呼叫，  
> 完全繞過上述初始化流程。結果是 LPAIF 驅動程式回應 `Invalid argument`——因為它尚未被 PAL 初始化，根本不知道要把資料送去哪裡。
>
> <font color="red">直接使用 TinyALSA 工具（tinyplay/tinycap）繞過 PAL 會導致 `Invalid argument`。</font>
>
> **結論**：tinyplay/tinycap 直接存取在此架構下不可用；請改用 PulseAudio/GStreamer 工具，正確地遵從路徑從上層存取。
...
aplay '/Wii_Music(48k).wav' -v
aplay '/Wii_Music(44.1k).wav' -v
```
...
# 使用者確認可成功播放的指令：
rubikpi_config audio output headset
gst-launch-1.0 filesrc location="/Wii_Music(48k).wav" ! \
